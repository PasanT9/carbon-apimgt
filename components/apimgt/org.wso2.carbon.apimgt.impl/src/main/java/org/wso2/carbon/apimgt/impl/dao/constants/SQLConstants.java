...
public class SQLConstants {
  ...
  //--------------------
    ...
    public static final String INSERT_LLM_PROVIDER_SQL = "INSERT INTO AM_LLM_PROVIDERS (UUID, NAME, VERSION, " +
            "DESCRIPTION, ORGANIZATION, MODEL_PATH, REQUEST_TOKEN_PATH, RESPONSE_TOKEN_PATH, TOTAL_TOKEN_PATH, " +
            "OPENAPI_DEFINITION) VALUES (?,?,?,?,?,?,?,?,?,?)";

    public static final String INSERT_LLM_PROVIDER_PARAMETERS = "INSERT INTO AM_LLM_PROVIDER_PARAMETERS " +
            "(LLM_PROVIDER_UUID, PARAMETER_NAME, PARAMETER_VALUE, IS_HEADER, IS_QUERY_PARAMS) VALUES (?,?,?,?,?)";

    public static final String GET_LLM_PROVIDERS_SQL = "SELECT UUID, NAME, VERSION, DESCRIPTION " +
            "FROM AM_LLM_PROVIDERS " +
            "WHERE ORGANIZATION = ?";

    public static final String GET_LLM_PROVIDER_SQL = "SELECT PROVIDER.UUID, PROVIDER.NAME, PROVIDER.VERSION, " +
            "PROVIDER.DESCRIPTION, PROVIDER.ORGANIZATION, PROVIDER.OPENAPI_DEFINITION, PARAMETER.PARAMETER_NAME, " +
            "PARAMETER.PARAMETER_VALUE, PARAMETER.IS_HEADER, PARAMETER.IS_QUERY_PARAMS " +
            "FROM AM_LLM_PROVIDERS PROVIDER JOIN AM_LLM_PROVIDER_PARAMETERS PARAMETER " +
            "ON PROVIDER.UUID = PARAMETER.LLM_PROVIDER_UUID WHERE PROVIDER.ORGANIZATION = ? AND PROVIDER.UUID = ?";

    public static final String DELETE_LLM_PROVIDERS_SQL = "DELETE FROM AM_LLM_PROVIDERS " +
            "WHERE ORGANIZATION = ? AND UUID = ?";

    public static final String UPDATE_LLM_PROVIDERS_SQL = "UPDATE AM_LLM_PROVIDERS SET NAME = ?, DESCRIPTION = ?, " +
            "OPENAPI_DEFINITION = ?, MODEL_PATH = ?, REQUEST_TOKEN_PATH = ?, RESPONSE_TOKEN_PATH = ?, " +
            "TOTAL_TOKEN_PATH = ? WHERE ORGANIZATION = ? AND UUID = ?";

    public static final String DELETE_LLM_PROVIDER_PARAMETERS_SQL = "DELETE FROM AM_LLM_PROVIDER_PARAMETERS " +
            "WHERE LLM_PROVIDER_UUID = ?";
    ...
    public static final String RETRIEVE_ALL_PER_API_RESOURCE_LOGGING_SQL =
            "SELECT API.API_UUID, AUM.LOG_LEVEL, API.API_NAME, API.CONTEXT, AUM.HTTP_METHOD, AUM.URL_PATTERN, " +
                    ...
                    "WHERE AUM.LOG_LEVEL <> 'OFF' AND AUM.REVISION_UUID IS NULL";
